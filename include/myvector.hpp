...
namespace mtd {
	...
	public:
		...
		// Logic Operator:
		template<typename Q>
		friend bool operator==(const vector<Q>& lhs, const vector<Q>& rhs);
	private:
		std::size_t size_;
	...
	template<typename Q>
	inline void vector<Q>::pop_front() {
		erase(1);
	}
	//-------------------------------------------------------------------------------------------------
	// 
	...
	inline void vector<Q>::insert(const std::size_t index, const Q value) {
		...
		new_region[index - 1] = value;
		std::copy(data_ + index - 1, data_ + size_, new_region + index);
	...
	template <typename Q>
	bool operator==(const mtd::vector<Q>& lhs, const mtd::vector<Q>& rhs) {
